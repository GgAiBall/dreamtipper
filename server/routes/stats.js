...
router.get('/overview', (req, res) => {
  try {
    ...
    const dailyStats = queryAll(`SELECT * FROM daily_stats ORDER BY date DESC LIMIT ?`, [parseInt(days)]).reverse();

    const leagueStats = queryAll(`
    ...
    const currentStreak = queryAll(`SELECT result FROM recommendations WHERE result != 'pending' ORDER BY published_at DESC LIMIT 20`);
