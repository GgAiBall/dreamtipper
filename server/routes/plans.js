...
router.get('/', (req, res) => {
  try {
    const plans = queryAll('SELECT * FROM plans WHERE is_active = 1 ORDER BY price ASC');
...
router.get('/:id', (req, res) => {
  try {
    const plan = queryOne('SELECT * FROM plans WHERE id = ? AND is_active = 1', [req.params.id]);
...
router.post('/:id/unlock', auth, (req, res) => {
  try {
    ...
    if (!plan) return res.status(404).json({ error: '方案不存在' });
    const purchased = queryOne(`SELECT * FROM user_purchases WHERE user_id = ? AND plan_id = ? AND status = 'paid'`, [req.params.user.id, req.params.id]);
    if (purchased) return res.json({ message: '已解锁', unlocked: true });
    const purchaseId = uuidv4();
    run(`INSERT INTO user_purchases (id, user_id, plan_id, type, amount, status, paid_at) VALUES (?, ?, ?, 'unlock', ?, 'paid', datetime('now'))`,
      [purchaseId, req.params.user.id, req.params.id, plan.price]);
    run('UPDATE plans SET subscriber_count = subscriber_count + 1 WHERE id = ?', [req.params.id]);
